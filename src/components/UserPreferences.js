...
export default function UserPreferences({ questionnaire, userId, updateSession }) {
  ...
  const savePrefUpdate = async (questionId, newValues) => {
    ...
    const exists = questionnaire.some(p => p.questionId === questionId);
    const updatedQuestionnaire = exists 
      ? questionnaire.map(p => p.questionId === questionId ? { ...p, answer: newValues } : p)
      : [...questionnaire, { questionId, answer: newValues }];
  ...
  return (
    <div className="flex flex-col gap-3 relative z-10">
      ...
      >
        ...
        <span className="text-[var(--text-muted)] text-[10px] font-bold ml-1">({questionnaire.length})</span>
        ...
        <div className="fixed inset-0 z-[10000] flex justify-end">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 flex-1">
              {OPTIONS_MAP[editingPref]?.map((opt) => {
